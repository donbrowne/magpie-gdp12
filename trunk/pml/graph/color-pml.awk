...
$7 == "unprovided" {
    # input?
    gsub("\'", "", $5);
    printf("/\\\"r:.*%s.*\\\"/s/fontcolor=\"BLACK\"/fontcolor=\"red\"/\n", $2);
}

$7 == "not" && $8 == "consumed" {
    # output?
    gsub("\'", "", $5);
    printf("/\\\"p:.*%s.*\"/s/labelfontcolor=\"black\"/labelfontcolor=\"green\"/\n", $2);
}
...
END {
    ...
    printf("a\\\nlegend5 [shape=plaintext, fontcolor=green, label=\"green text - output?\"];\n");
    printf("a\\\nlegend6 [shape=plaintext, fontcolor=red, label=\"red text - input?\"];\n");
