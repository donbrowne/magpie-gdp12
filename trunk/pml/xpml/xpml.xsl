...
<xsl:template match="document">
  <xsl:apply-templates select="body/process"/>
</xsl:template>

<xsl:template match="process">
  <xsl:text>process </xsl:text>
  <xsl:value-of select="translate(/document/@name, '-', '_')"/>
  <xsl:text> {
</xsl:text>
...
<xsl:template match="title|subtitle|author|institution|abstract|section">
</xsl:template>
